...
try {
    ...
    $stmt = $conn->prepare("SELECT * FROM pessoas limit $inicio, $quantidade");
    $stmt->execute();
        ...
        <ul class="pagination">
            <?php 
                for ($i = 1; $i <= $numero_pagina; $i++) { ?>
                    <li class="page-item"><a class="page-link" href="?modulo=pessoas&pagina=listagem&paginacao=<?php echo $i; ?>"><?php echo $i; ?></a></li>
            <?php    }
            ?>
        </ul>
    </nav>
